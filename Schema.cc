...
Attribute::Attribute() :
        name(""), type(Name), noDistinct(0), changed(false)
{
}
...
Schema::Schema(vector<string>& _attributes, vector<string>& _attributeTypes, vector<unsigned int>& _distincts)
{
    for (int i = 0; i < _attributes.size(); i++)
    {
        ...
        if (_attributeTypes[i] == "Integer" || _attributeTypes[i] == "INTEGER")
            a.type = Integer;
        else if (_attributeTypes[i] == "Float" || _attributeTypes[i] == "FLOAT")
            a.type = Float;
        else if (_attributeTypes[i] == "String" || _attributeTypes[i] == "STRING")
            a.type = String;
