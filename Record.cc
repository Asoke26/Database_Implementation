...
void Record::CopyFrom(Record& _copyMe) {
    *this=_copyMe;
}
...
//            int t=((int *) rec_bits)[0];
//            int t1=((int *) rec_bits)[attsToKeep[i] + 1];

			attLen = ((int *) rec_bits)[0] - ((int *) rec_bits)[attsToKeep[i] + 1];
		}
		else {
			// subtract the start of the next field from the start of this field
//			int t=((int *) rec_bits)[attsToKeep[i] + 2];
//            int t1=((int *) rec_bits)[attsToKeep[i] + 1];
...
string Record :: findKey(Schema& mySchema) {
	int n = mySchema.GetNumAtts();
	vector<Attribute> atts = mySchema.GetAtts();
	stringstream ss;
