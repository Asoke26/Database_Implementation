...
bool Catalog::CreateTable(string& _table, vector<string>& _attributes, vector<string>& _attributeTypes)
{
    ...
    string _cwd(cwd);
    _cwd+="/tbl_files/"+_table+".tbl";

    tableData.setName(_table);
    tableData.setTuples(0);
    tableData.setPath(_cwd);
...
/*
bool CreateNewTable(string& _table, vector<string>& _attributes, vector<string>& _attributeTypes,int i)
{
    string sql="CREATE TABLE "+_table+" ( ";
    for (int i = 0; i <_attributes.size() ; ++i) {
        sql+=_attributes[i]+" ";
        if(i!=_attributeTypes.size()-1)sql+=_attributeTypes[i]+" ,";
        else{
            sql+=_attributeTypes[i]+" );";
        }
    }
