...
//	return _os << "  SCANING[" << schema << "]";
    return _os << "  SCANING[SOURCE TABLE]";
}
...
Join::Join(Schema& _schemaLeft, Schema& _schemaRight, Schema& _schemaOut,
	...
	predicate(_predicate),left(_left),right(_right),buildPhase(0),joinTrue(false),leftActive(true),leftCount(0),rightActive(
                false),rightCount(0),returnActive(false),allrecdiscoveredLeft(false),allrecdiscoveredRight(false){
...
//    OML=OrderMaker(schemaLeft);
    ...
    //// Find The join attribute and set orderMaker for Left Child
        ...
        if (c.operand1 == Left) {
            ...
            for (int j = 0; j < schemaLeft.GetNumAtts(); ++j) {
                if(j==pos)LeftAttr=attListLeft[j];
            }
            ...
            OMR=OrderMaker(schemaRight,keepMe,1);
             ...
             /// Getting the attribute
            for (int j = 0; j < schemaRight.GetNumAtts(); ++j) {
                if(j==pos)RightAttr=attListRight[j];
            }
        }
        ...
        if (c.operand2 == Left) {
            ...
            OML=OrderMaker(schemaLeft,keepMe1,1);
             ...
             /// Getting the attribute
            for (int j = 0; j < schemaLeft.GetNumAtts(); ++j) {
                if(j==pos1)LeftAttr=attListLeft[j];
            }
            ...
            OMR=OrderMaker(schemaRight,keepMe1,1);
              ...
              /// Getting the attribute
            for (int j = 0; j < schemaRight.GetNumAtts(); ++j) {
                if(j==pos1)RightAttr=attListRight[j];
            }
        }
        ...
        if(c.op== LessThan || c.op==GreaterThan) whichJoin=0;
        else{
            if(LeftAttr.noDistinct > 1000 && RightAttr.noDistinct > 100) whichJoin=2;
            else whichJoin=1;
        }
}
...
ostream& Join::print(ostream& _os)
{
	...
	for (int i = 0; i < predicate.numAnds; i++) {
            ...
            if(whichJoin==0)_os<<" NLJ ";
            if(whichJoin==1)_os<<" HJ ";
            if(whichJoin==2)_os<<" SHJ ";
...
bool Join::GetNext(Record &_record){
    ...
    Record comp;comp.Nullify();
    if(whichJoin==0) {

        while(NLJGetNext(_record)){
            return true;
        }
        return false;
    }
    else if(whichJoin==1) {
        while(HJGetNext(_record)){
            return true;
        }
...
bool Join::SHJGetNext(Record &_record)
{

    while ( true){
        if(leftActive && !returnActive){
            Record tempRec;
            while (left->GetNext(tempRec)){
                ...
                if (HT_L.IsThere(tempRec)) {
                    ...
                    runningRec.Append(key2);   /// Temporary store for storing running Records
                }
                leftCount++;
                if(allrecdiscoveredRight== false && leftCount==10) {
                    break;
                }
                ...
                while(runningRec.RightLength()!=0){
                    ...
                    if (HT_R.IsThere(key)) {
                        ...
                        while ( TempStore.RightLength() != 0 ) {
                            Record joinRec;
                            joinRec.MergeRecords(key,TempStore.Current(), schemaLeft.GetNumAtts(),
        ...
        if(rightActive && !returnActive){
            ...
            while (right->GetNext(tempRec)){
                ...
                rightCount++;
                if(allrecdiscoveredLeft==false && rightCount==10) {
                    break;
                }
        ...
        if(!returnActive && allrecdiscoveredRight && allrecdiscoveredLeft) return false;
    }
}
...
bool Join::HJGetNext(Record &_record) {
...
bool Join::NLJGetNext(Record &_record){
