...
//    EfficientMap <KeyString , TwoWayList<Record>> HT,HT_R,HT_L;  // For Key String Based Implementation
    ...
    TwoWayList<Record> TempReturn,runningRec,absorb;
    OrderMaker OML,OMR;

    int whichJoin; /// 0 = Nested Loop 1= Hash 2=Symmetric Hash
    Attribute LeftAttr,RightAttr;
    bool leftActive,rightActive,returnActive,allrecdiscoveredLeft,allrecdiscoveredRight;
...
public:
    ...
    bool NLJGetNext(Record& _record);
    bool HJGetNext(Record& _record);
    bool SHJGetNext(Record& _record);
